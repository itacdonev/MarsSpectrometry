...
def bin_temp_abund(df_sample, sample_name):
    """
    ...
    return ht_pivot.fillna(0)


def features_iontemp_abun(df_meta, sample_list:list):
    ...
    for i, sample_name in tqdm(enumerate(sample_list)):
        df_sample = preprocess.get_sample(df_meta, i)
        
        ht_pivot = bin_temp_abund(df_sample, sample_name)
        dt = pd.concat([dt, ht_pivot], ignore_index=True)
